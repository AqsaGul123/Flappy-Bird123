...
# pygame.display.set_caption('Flappy Bird By Twiny Girls')
...
screen = pygame.display.set_mode((screen_width, screen_height))
pygame.display.set_caption('Flappy Bird By Aqsa Gul')
...
hit_sound = pygame.mixer.Sound('img/Hit2.wav')
    ...
    def update(self):
        ...
        if not game_over:
            ...
            if pygame.mouse.get_pressed()[0] == 1 and not self.clicked:
                self.clicked = True
                self.vel = -10
                wing_sound.play()  # Play wing sound on jump
            if pygame.mouse.get_pressed()[0] == 0:
                self.clicked = False
...
while run:
    ...
    if len(pipe_group) > 0:
        ...
        if pass_pipe:
            if bird_group.sprites()[0].rect.left > pipe_group.sprites()[0].rect.right:
                score += 1
                pass_pipe = False
                point_sound.play()  # Play point sound when passing a pipe
        draw_text(str(score), font, black, int(screen_width / 2), 20)        
    ...
    if not game_over and flying:
        ...
        if time_now - last_pipe > pipe_frequency:
            ...
            pipe_group.add(top_pipe)
            last_pipe = time_now
            #swoosh_sound.play()  # Play swoosh sound when new pipes are generated
